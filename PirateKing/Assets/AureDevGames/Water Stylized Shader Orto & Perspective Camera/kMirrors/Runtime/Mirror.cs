        ...
        ProfilingSampler profilingSampler
        {
            get
            {
                if(m_ProfilingSampler == null)
                    m_ProfilingSampler = new ProfilingSampler($"Mirror {gameObject.GetEntityId()}");
...
#region State
        ...
        {
            // Callbacks
            // NOTE: Mirror reflections are temporarily disabled. kMirrors 0.1.0 was built for
            // Unity 2019.3 / URP 7.x, and its BeginCameraRendering call into
            // UniversalRenderPipeline.RenderSingleCamera is incompatible with the Render Graph-based
            // pipeline in URP 17 (Unity 6), throwing a NullReferenceException in
            // RenderSingleCameraInternal. Re-enable this subscription once the asset (or this script)
            // is updated to support Render Graph.
